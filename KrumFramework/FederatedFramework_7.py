...
def Update():
    ...
    Train(network, sample_training, label_training, round_epoch, range_gradient_clipping, rate_gradient_clipping,
          threshold_differential, threshold_loss, rate_annealing)
    w = []
...
# This node is a servant node

@app.route('/Poisoning', methods=['POST'])
def Poisoning():
    for i in ip:
        try:
            t = threading.Thread(target=Broadcast, args=(request.form, i))
            t.start()
        except:
            print('Failed to broadcast parameters to 192.168.0.{}'.format(i))
    return ''
...
if __name__ == '__main__':
    lock = threading.Lock()
    app.run(host='{}'.format(addr), port=9007)