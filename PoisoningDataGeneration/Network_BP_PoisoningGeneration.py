...
class Network:
    ...
    def SampleOptimization(self, array_input: list, array_label: list):
        # NOTE: Take the whole sample matrix as input
        a_i = []
        x_c=cp.array(array_input.copy())
        y_c=cp.array(array_label.copy())
        ...
        if len(layer_current[0]) != len(self.list_weight[0][0]) - 1:
            ...
            raise Exception("The size of label vector should be {}".format(len(self.list_weight[-1])))
        for m in self.list_weight:
            layer_current = cp.asarray(
                np.append(cp.asnumpy(cp.ones((len(array_input), 1))), cp.asnumpy(layer_current), axis=1))
            a_i.append(layer_current)
            layer_current = cp.matmul(layer_current, cp.asarray(m.transpose(), dtype='float32'))
            layer_current = self.Neuron(layer_current)
        # print(layer_current)
        ...
        # last-level backward propagation to compute the derivative of theta_n-1
        self.iteration += 1
        loss = cp.subtract(a_i[-1], label_current)
        loss_sum = cp.sum(cp.abs(loss))
        derivative_ReLu = self.Neuron_D(a_i[-1])
        rate_error = cp.multiply(loss, derivative_ReLu)
        gradient_current_average = cp.divide(cp.matmul(rate_error[:, :, None], a_i[-2][:, None]).sum(axis=0),
                                             len(array_input))
        SIGMA=cp.divide(cp.matmul(a_i[-2].transpose()[:,:,None], a_i[-2][:,None]).sum(axis=0), len(a_i[-2]))
        Mu=cp.mean(a_i[-2].sum(axis=0),axis=0)
    ...
    for i, row in enumerate(f_csv):
        ...
        if i % size_batch == 0:
            ...
            if normalization:
                ...
                mu = cp.mean(sample_temp_2, axis=0)
                sigma = cp.std(sample_temp_2, axis=0)
                sample_temp_2 = (sample_temp_2 - mu) / sigma
