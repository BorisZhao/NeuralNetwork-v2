...
def FederatedAverage():
    ...
    if len(list_w)<C*K:
        ...
        if len(list_w)==C*K:
            ...
            update = multiprocessing.Process(target=Update, args=(
            weight, train_loss, test_loss, sample_training, label_training, sample_test, label_test, queue))
            update.start()
    return ''
